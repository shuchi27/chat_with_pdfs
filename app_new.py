...
def model_query(user_question,num_sources,faiss_indices,selected_file,vectorStore):
...
def choose_chain(token_count, model_token_limit):
...
def main():
        ...
        if st.button("Load Data"):
            ...
            if isFile:
                for f in files:
                    ...
                    db = load_vectorstore(text_chunks)
            ...
            if st.button("Summary"):
                ...
                with st.spinner("Processing..."):
                    ...
                    template_string = "read the text delimited by three angular brackets.\
                        your task is to read the delimited list of text, extract most relevent information which summarize the whole document.exclude the disclaimer from the summary if any exists in the document. \
                    ...
                    if faiss_indices is not None:
                        ...
                        summary_response =  chain.run(all_chunks)
   ...
   # Lower section for prompt input
    ...
    if user_question:
        ...
        model_response, context = model_query(user_question,num_sources,faiss_indices,selected_file,db)
