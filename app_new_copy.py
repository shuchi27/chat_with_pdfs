...
def model_query(user_question,num_sources,faiss_indices,selected_file,vectorStore,index_type,vectors,vector_ids):
    ...
    custom_retriever = CustomRetriever(vector_store=new_db, num_results=num_sources)
...
def get_indices_for_file(db, file_name):
    # Variables to store keys, documents, and their embeddings
    document_keys = []
    embeddings = []
    documents = []

    # Generate embeddings and collect documents
    for key, document in db.docstore.items():
        if document.metadata.get('source') == file_name:
            document_keys.append(key)
            documents.append(document)
            if hasattr(document, 'embedding'):
                embeddings.append(document.embedding)
            else:
                embeddings.append(db.embedding_function(document.content))

    # Check if no documents are found
    if not documents:
        raise ValueError(f"No documents found for the file: {file_name}")

    # Create a new FAISS index from these embeddings
    new_index = FAISS.from_embeddings(embeddings)

    # Create a new document store from the filtered documents
    new_docstore = {key: doc for key, doc in zip(document_keys, documents)}

    # Create a new FAISS database object
    new_db = FAISS(
        index=new_index,
        docstore=new_docstore,
        index_to_docstore_id=document_keys,
        embedding_function=db.embedding_function  # Reuse the original embedding function
    )
