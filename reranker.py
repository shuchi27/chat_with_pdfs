...
def model_query(user_question, num_sources, vectorStore, index_type):
   ...
   #context = "\n\n".join([doc.page_content for doc in relevant_docs])
    ...
    queries = [
        "What is the primary goal of determining the optimal chunk size in embedding calculations and FAISS database operations?",
        "What are the advantages and disadvantages of using bigger chunks in embedding calculations?",
        "How do smaller chunks impact FAISS database operations?",
        "What are the pros and cons of using bigger chunks in FAISS database operations?",
        "Why is it important to benchmark different chunk sizes?",
        "What should be monitored to avoid slowdowns during embedding calculations?",
        "What are the advantages of using word embeddings for titles compared to TFIDF?",
        "What are the memory and computation time requirements for TFIDF compared to word embeddings?",
        "What method was used for visualizing the journal embeddings?",
        "What configurations of TFIDF were selected for the research, and why?",
        "What benchmarks were used for evaluating word similarity during model optimization?",
        "What are the parameters of the final word embedding model used in the study?",
        "What BLEU scores did the Transformer achieve on the WMT 2014 English-to-German and English-to-French translation tasks?"
    ]
    ground_truths = [
        "The primary goal is to balance efficiency and speed when using OpenAI embedding models and a FAISS database, particularly on a Mac laptop without a GPU.",
        "Pros: Reduced overhead with fewer chunks. Cons: Longer processing time per chunk.",
        "Pros: Typically easier to manage in memory. Cons: More entries in the FAISS database, potentially slowing down searches.",
        "Pros: Fewer entries in the FAISS database, potentially speeding up search operations. Cons: Requires more memory per chunk.",
        "Benchmarking different chunk sizes helps in measuring the total time taken to process and store embeddings for different chunk sizes, allowing for comparison to find the optimal balance.",
        "Memory usage should be monitored to ensure the system’s capacity is not exceeded, which can prevent swapping and slowdowns.",
        "Word embeddings perform better for titles (short text) as compared to TFIDF.",
        "TFIDF requires 3.7 times more memory and up to 184 times higher computation times compared to word embeddings.",
        "A 2-dimensional visualization using PCA-based tSNE (t-Stochastic Neighbor Embedding) was used for visualizing the journal embeddings.",
        "The selected configurations were TFIDF 10K/10K, 10K/5K, and 5K/5K, as they offered reasonable compromises between memory footprint and accuracy.",
        "The benchmarks used were the WordSimilarity-353, MEN, and the Rare Word test collections.",
        "The final word embedding model is based on 300-dimensional vectors, a context window of 5 tokens, 1 iteration, and 160 partitions.",
        "The Transformer achieved a BLEU score of 28.4 on the WMT 2014 English-to-German translation task and 41.8 on the WMT 2014 English-to-French translation task."
    ]
